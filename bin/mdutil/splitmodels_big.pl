...
models are numbered increasingly, regardless of model # in file
removes H
";
...
sub pdb_fields {
    ...
    if ( $r{ "recname" } eq "LINK" ) {
        ...
        # $r{ "model"     } = mytrim( substr( $l, 10, 4 ) ) + 1; # dcd conversion can have  model numbers of zero
        $r{ "model"     } = ++$model_no;
    }
    \%r;
}
...
foreach $l ( @l ) {
    ...
    if ( $r->{"recname"}  =~ /^MODEL$/ ) {
        if ( @ol && $model ) {
            my $fo = sprintf( ">$bn-m%s.pdb", myleftpad0( $model, 5 ) );
